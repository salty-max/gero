...
#   DOC       — the document to check (default docs/asm.md)
#
# Exit codes:
...
# Split the document into one file per ```asm block, named by the line
# the block opens on so a failure points back at the source.
awk -v dir="$work" '
...
checked=0
failed=0
for f in "$work"/*.gas; do
    [[ -e "$f" ]] || continue
    line="$(basename "$f" .gas)"
    if head -n 1 "$f" | grep -q '^; fragment:'; then
        continue
    fi
    # A block without a label is a body: give it an entry point so it
    # stands alone, the way the surrounding prose implies.

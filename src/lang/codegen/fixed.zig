...
pub fn emitBinary(self: *Emitter, b: ast.BinaryExpr) error{OutOfMemory}!void {
    if (b.op == .div and try tryEmitConstDiv(self, b)) return;
...
/// Lower division by an exact power-of-two literal without calling the
/// restoring-division helper. Returns `false` for every other divisor.
pub fn tryEmitConstDiv(self: *Emitter, b: ast.BinaryExpr) error{OutOfMemory}!bool {
    if (b.rhs.* != .fixed_lit) return false;
    const raw = b.rhs.fixed_lit.value;
    if (raw == 0) return false;

    const wide: i64 = raw;
    const magnitude: u32 = @intCast(if (wide < 0) -wide else wide);
    if (magnitude & (magnitude - 1) != 0) return false;

    var exponent: i8 = -16;
    var probe = magnitude;
    while (probe > 1) : (probe >>= 1) exponent += 1;

    try self.emitExpr(b.lhs);
    if (exponent > 0) {
        try emitShiftRightTowardZero(self, @intCast(exponent));
    } else if (exponent < 0) {
        try emitShiftLeft(self, @intCast(-exponent));
    }
    if (raw < 0) try emitNegatePair(self, lo, Emitter.fixed_hi);
    return true;
}

/// Shift a signed pair right while preserving division's round-toward-zero
/// rule for negative values with discarded bits.
fn emitShiftRightTowardZero(self: *Emitter, amount: u8) !void {
    // @as: the shift amount is at most 15, so its low-bit mask fits a word.
    const mask: u16 = (@as(u16, 1) << @intCast(amount)) - 1;
    try isa.movImmToReg(self, 0, Reg.r4); // adjustment flag
    try isa.cmpRegImm(self, Emitter.fixed_hi, 0);
    const nonnegative = try isa.emitJumpPlaceholder(self, Op.jge_addr);
    try isa.movRegToReg(self, lo, Reg.r3);
    try isa.movImmToReg(self, mask, Reg.r6);
    try isa.andRegReg(self, Reg.r3, Reg.r6);
    try isa.cmpRegImm(self, Reg.r3, 0);
    const exact = try isa.emitJumpPlaceholder(self, Op.jeq_addr);
    try isa.movImmToReg(self, 1, Reg.r4);
    const shift = try self.currentOffset();
    try isa.patchJumpTo(self, nonnegative, shift);
    try isa.patchJumpTo(self, exact, shift);

    try isa.movRegToReg(self, Emitter.fixed_hi, Reg.r3);
    try isa.shlRegImm(self, Reg.r3, 16 - amount);
    try isa.shrRegImm(self, lo, amount);
    try isa.orRegReg(self, lo, Reg.r3);
    try isa.asrRegImm(self, Emitter.fixed_hi, amount);

    try isa.cmpRegImm(self, Reg.r4, 0);
    const done = try isa.emitJumpPlaceholder(self, Op.jeq_addr);
    try isa.addImmToReg(self, 1, lo);
    try isa.adcImmToReg(self, 0, Emitter.fixed_hi);
    try isa.patchJumpTo(self, done, try self.currentOffset());
}

/// Shift a two-word value left, keeping the low 32 bits.
fn emitShiftLeft(self: *Emitter, amount: u8) !void {
    if (amount == 16) {
        try isa.movRegToReg(self, lo, Emitter.fixed_hi);
        try isa.movImmToReg(self, 0, lo);
        return;
    }
    try isa.movRegToReg(self, lo, Reg.r3);
    try isa.shrRegImm(self, Reg.r3, 16 - amount);
    try isa.shlRegImm(self, Emitter.fixed_hi, amount);
    try isa.orRegReg(self, Emitter.fixed_hi, Reg.r3);
    try isa.shlRegImm(self, lo, amount);
}
...
pub fn emitOperands(self: *Emitter, b: ast.BinaryExpr) error{OutOfMemory}!void {
    return emitExprOperands(self, b.lhs, b.rhs);
}

/// Evaluate two fixed expressions into the pair convention used by the
/// arithmetic and comparison helpers.
pub fn emitExprOperands(self: *Emitter, lhs: *const ast.Expr, rhs: *const ast.Expr) error{OutOfMemory}!void {
...
/// Load a Q16.16 pair from `base + offset` into `acu` / `r5`.
pub fn loadPairAt(self: *Emitter, base: u8, offset: u16) !void {
    const stable_base = if (base == lo) Reg.r1 else base;
    if (stable_base != base) try isa.movRegToReg(self, base, stable_base);
    try isa.movRegOffsetToReg(self, stable_base, @intCast(offset), lo);
    try isa.movRegOffsetToReg(self, stable_base, @intCast(offset + 2), Emitter.fixed_hi);
}

/// Store a Q16.16 pair at `base + offset`, low word first.
pub fn storePairAt(self: *Emitter, base: u8, offset: u16, low_reg: u8, high_reg: u8) !void {
    try isa.movRegToRegOffset(self, low_reg, base, @intCast(offset));
    try isa.movRegToRegOffset(self, high_reg, base, @intCast(offset + 2));
}

/// Whether a checked type is the `fixed` primitive.
pub fn isFixedType(ty: *const types.Type) bool {
    return ty.* == .primitive and ty.primitive == .fixed;
}

...
pub const Emitter = struct {
    ...
    fn countExprInlineBytes(self: *const Emitter, e: *const ast.Expr, depth: u8) usize {
        return switch (e.*) {
            ...
            // A lambda body is a separate def with its own prologue.
            // A `do … end` value block (§4.3) and an `if` chain in
            // value position (§4.4.2) both reserve their inner locals
            ...
            .if_expr => |ie| blk: {
                var n: usize = 0;
                for (ie.arms) |a| {
                    if (a.let_pattern) |lp| n += self.letArmFrameBytes(lp, a.let_expr);
                    n += self.countFrameBytesDepth(a.body, depth);
                }
                if (ie.else_body) |eb| n += self.countFrameBytesDepth(eb, depth);
                break :blk n;
            },
            .str_lit => |s| blk: {
                var n: usize = 0;

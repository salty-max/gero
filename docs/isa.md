...
| `0x1200..0x7FFF` | 27.5 KB | **User RAM** — code, data, heap, and the stack. The image is loaded from `0x0000`, so this is where a program's code actually begins to sit; `.gr` puts code at `0x1200` and static data at `0x2000`, or word-aligned just above the code when the code extends past `0x2000` — the two regions never overlap, and a program too large for both to fit below `0xFE40` is rejected with `E_CODEGEN_IMAGE_OVERFLOW`. `sp` boots at `0x7FFE` and grows **down** toward the heap growing **up**. |
